...
void dijkstraGPU(GraphData *graph, const int sourceVertex, float * __restrict__ h_shortestDistances) {
	...
	while (!allFinalizedVertices(h_finalizedVertices, graph->numVertices)) {
		...
		for (int asyncIter = 0; asyncIter < NUM_ASYNCHRONOUS_ITERATIONS; asyncIter++) {
			...
			Kernel1 << <((float)(graph->numVertices)/BLOCK_SIZE), BLOCK_SIZE >> > (d_vertexArray, d_edgeArray, d_weightArray, d_finalizedVertices, d_shortestDistances,
				d_updatingShortestDistances, graph->numVertices, graph->numEdges);

			cudaDeviceSynchronize();
			Kernel2 << <((float)(graph->numVertices) / BLOCK_SIZE), BLOCK_SIZE >> > (d_vertexArray, d_edgeArray, d_weightArray, d_finalizedVertices, d_shortestDistances, d_updatingShortestDistances,
				graph->numVertices);
			cudaDeviceSynchronize();
		}
